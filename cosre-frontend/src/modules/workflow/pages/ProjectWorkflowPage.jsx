...
import { assignProjectToClassroom, createSyllabus, getApprovedProjects, getClassrooms, getMyProjects, getReviewProjects, getSubjects, getSyllabi, reviewProject } from '../workflowService';
...
export default function ProjectWorkflowPage() {
  ...
  const assign = async (project) => {
    ...
    if (!raw) return;
    try { await assignProjectToClassroom(project.id, Number(raw)); setFeedback('Đã gán dự án cho lớp.'); }
    catch (error) { setFeedback(getApiError(error)); }
  };

  return <DashboardShell role={role} displayName={displayName} activePath="/workflow" pageTitle="Luồng dự án">
    <div className="workflow-page"><header><div><span>CORE WORKFLOW</span><h1>{role === 'STAFF' ? 'Đề cương môn học' : 'Phê duyệt & phân công dự án'}</h1><p>Luồng liên kết dữ liệu từ đề cương đến lớp và nhóm sinh viên.</p></div></header>
      ...
      : <section className="workflow-panel"><h2><FolderKanban size={18}/> Danh sách dự án</h2>{projects.length === 0 ? <p>Chưa có dự án phù hợp.</p> : projects.map((p) => <article className="workflow-card" key={p.id}><div><b>{p.title}</b><span className={`status ${p.status.toLowerCase()}`}>{statusLabel[p.status]}</span></div><small>Subject #{p.subjectId} · Syllabus #{p.syllabusId || '—'} · Lecturer #{p.createdBy}</small><p>{p.description}</p><ul>{p.milestones.map((m) => <li key={m.id || m.title}>{m.title} ({m.dueOffsetDays} ngày)</li>)}</ul><footer>{role === 'HEAD_DEPT' && p.status === 'PENDING' && <><button onClick={() => decide(p, true)}><Check size={15}/> Duyệt</button><button className="danger" onClick={() => decide(p, false)}><X size={15}/> Từ chối</button></>}{p.status === 'APPROVED' && <button onClick={() => assign(p)}><Link2 size={15}/> Gán cho lớp</button>}</footer></article>)}</section>}
    </div></DashboardShell>;
}
