...
function TeamGrades({ team }) {
  ...
  return <><Notice error={error || notice.error} message={notice.message} />{error && <button onClick={reload}>Thử lại</button>}
    ...
    {role === 'STUDENT' && <p>{round.locked ? 'Đợt đã kết thúc. Bạn có thể xem kết quả dưới đây.' : round.finalOpen ? <Link to="/peer-evaluations">Chấm đánh giá chéo cho đồng đội</Link> : 'Giảng viên chưa mở đợt đánh giá.'}</p>}</section>
