...
export default function AIChatWidget() {
  ...
  return <div className="cosre-ai-widget">
    {open && <section className="cosre-ai-widget-panel" aria-label="COSRE AI Chatbot">
      ...
      <div className="cosre-ai-widget-messages">{messages.map((message, index) => <div key={`${message.role}-${index}`} className={`cosre-ai-message ${message.role}`}>{message.text}</div>)}{loading && <div className="cosre-ai-message assistant typing">Đang suy nghĩ…</div>}</div>
