...
import { addTeamMember, assignTeamProject, createTeam, deleteTeam, getAvailableStudents, getTeams, removeTeamMember } from '../teamService';
...
function TeamManagementPage() {
  ...
  const submit = async (event) => {
    event.preventDefault(); setSaving(true); setFeedback({ type: '', text: '' });
    try {
  ...
  const chooseProject = async (team) => {
    const raw = window.prompt('Nhập ID đề tài đã được duyệt:', team.projectId || '');
    if (!raw) return;
  ...
  return (
    <main className="team-shell">
      ...
      {showForm && <div className="team-modal-backdrop" onMouseDown={() => setShowForm(false)}><form className="team-modal" onSubmit={submit} onMouseDown={(event) => event.stopPropagation()}><div className="team-modal-title"><div><span>TẠO NHÓM</span><h2>Thông tin nhóm mới</h2></div><button type="button" onClick={() => setShowForm(false)}><X /></button></div>
        ...
        <div className="team-form-row"><label>ID lớp học<input required min="1" type="number" value={form.classroomId} onChange={(event) => { setStudents([]); update('classroomId', event.target.value); }} /></label><label>ID đề tài (tùy chọn)<input min="1" type="number" value={form.projectId} onChange={(event) => update('projectId', event.target.value)} /></label></div>
        ...
        <div className="team-modal-actions"><button type="button" onClick={() => setShowForm(false)}>Hủy</button><button className="team-primary" disabled={saving}>{saving ? 'Đang tạo…' : 'Tạo nhóm'}</button></div>
      </form></div>}
    </main>
  );
}
